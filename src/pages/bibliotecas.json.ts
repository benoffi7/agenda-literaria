 ...
 * lógica con la que el panel se corta del bundle público (§9), y es la que ya
 * siguen los otros tres directorios.
 *
 * ── La URL no lleva `/guia/` ─────────────────────────────────────────────
 * `/guia/` es una decisión de navegación y de SEO, y esto **no es una página
 * navegable**: es un artefacto, como `/events.json` y `/version.json`.
 *
 * ── La lectura no vive acá ───────────────────────────────────────────────
 * Igual que `events.json.ts` y `sitemap.xml.ts`: **qué documentos se leen** lo
 * decide el lector (`contenidoDelSitio.ts`, con su
 * `where('estado','==','publicado')` y la cláusula de credenciales de D-123),
 * **qué sale de cada uno** lo decide la proyección (`bibliotecaPublica.ts`, una
 * whitelist), y acá solo se serializa. Sin ese corte, la cláusula del estado se
 * copiaría una vez más y la copia que se olvide publica lo pendiente — con el
 * contacto interno de quien lo cargó.
 *
