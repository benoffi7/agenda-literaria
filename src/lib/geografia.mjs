 ...
 *
 * ── Por qué esto es `.mjs` ────────────────────────────────────────────────
 * Por lo mismo que `slugify.mjs` y `ciudades.mjs`: el backfill
 * (`scripts/sembrar-geografia.mjs`) corre en node y tiene que derivar
 * **exactamente lo mismo** que el panel. Con una copia en el script, un documento
 * sembrado y uno guardado desde el panel podrían discrepar en el slug de la
 * provincia, y el síntoma sería una sede que se filtra desde un lado y no desde
 * el otro.
 */
