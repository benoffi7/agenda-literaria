...
// B-831 — la proyección de cada ficha de la Guía es **por entidad** y no
// genérica (§5.2): una whitelist escrita a mano por colección, en
// `lib/<entidad>Publica.ts`. Lo que el build deriva de ellas vive en
// `lib/contenidoDeLaGuia.ts` (M-9); acá queda solo la lectura. La de lugares
// tiene algo que las otras no: la dirección sale o no sale según un flag del
// documento (`dondeQueSale`, § 6 del PRD 4).
import { libreriaPublica, type LibreriaPublica } from '@/lib/libreriaPublica';
import { suscripcionPublica, type SuscripcionPublica } from '@/lib/suscripcionPublica';
import { lugarPublico, type LugarPublico } from '@/lib/lugarPublico';
...
import { bibliotecaPublica, type BibliotecaPublica } from '@/lib/bibliotecaPublica';
