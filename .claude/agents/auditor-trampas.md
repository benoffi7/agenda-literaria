...
| **Saneador campo por campo** (B-81, B-137) | `clases-de-bug` — tres asertos desde B-137 (2026-09-02): un centinela **saneable** en cada string de la entrada del issue, un centinela que el saneador **no** tapa para los campos que protege la enumeración y no el filtro, y un **tope de dos aplicaciones** de `redactar` en el cuerpo de `construirIssue`, que es el que impide volver al reparto | el tope y los dos centinelas son automáticos; lo que sigue **a mano** es el resto de las salidas | una interpolación nueva en una salida pública que no pase por el saneador, y sobre todo **una transformación entre la entrada y el punto de paso**: `desSlug()` corre aguas arriba y le mete un espacio al medio del patrón de link (B-363), así que el punto de paso único no alcanza cuando algo toca el texto antes. Es de `auditor-privacidad` si lo que se filtra es contenido: nombralo y derivá |
