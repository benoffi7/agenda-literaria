...
## Las treinta salidas, y de qué archivo sale cada una
...
Las treinta de arriba son **productoras**: proyectan o emiten. Estas otras son
...
Y una regla de forma que vale para las treinta salidas: **si la salida se arma
...
7. Si el cambio agrega una **salida nueva** (un endpoint, un webhook, un log
   con contenido, un mail, un JSON más, **una página**), decilo fuerte: son **treinta**
   hoy y una trigésima cambia el mapa y la doc — esta tabla, la de
...
2. **Tabla de campos tocados × las treinta salidas** (`sale` / `no sale` /
