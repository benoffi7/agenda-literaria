...
## Las cinco salidas, y de qué archivo sale cada una
...
`urlPublica: true`**. El flag de D-15 alcanza **solo en las salidas 1 y 2**; a la
3, a la 4 y a la 5 la URL no llega nunca, con flag o sin flag.
...
- `online.url` — salvo `online.urlPublica === true`, y **solo** a las salidas 1
  y 2 (desvío deliberado, D-15). A las salidas 4 y 5 no va **nunca**, ni con el
  flag en true: quien tilda la casilla está autorizando el sitio y el calendario,
  no la analítica ni un posteo abierto. Sin URL cargada no se inventa el campo.
...
Y una regla de forma que vale para las cinco salidas: **si la salida se arma
...
1. Mirá el cambio: `git diff --stat` y `git diff` (o los archivos que te
   nombren). Si no hay diff, auditá los cinco archivos productores completos.
2. **Por cada campo nuevo o modificado del modelo**, resolvé las cinco celdas:
   ¿va a 1? ¿a 2? ¿a 3? ¿a 4? ¿a 5? Un campo sin las cinco respuestas es un hallazgo
   por sí mismo: nadie decidió, y el default de "lo agrego al `pick`" publica.
   Y una **sexta** pregunta, que es la que decide si el campo es interno: **quién
   lo escribe.** Si lo escribe una Cloud Function, es candidato a no salir a
   ninguna de las cinco (como `calendarEventId`), y su conflicto de dueños con
   el formulario es de `auditor-trampas` — nombralo y derivá.
3. **Verificá la forma de la proyección, no solo el contenido.** Estas cinco
...
7. Si el cambio agrega una **salida nueva** (un endpoint, un webhook, un log
   con contenido, un mail, un JSON más), decilo fuerte: son cinco hoy y una
   sexta cambia el mapa y la doc.
...
2. **Tabla de campos tocados × las cinco salidas** (`sale` / `no sale` /
