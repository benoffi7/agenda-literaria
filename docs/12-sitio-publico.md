...
| `/guia` | El índice de los cuatro directorios (eran tres hasta B-960). **No estaba en este diseño** — B-835, decisión del dueño del 2026-09-08. Las filas salen de `DIRECTORIOS` (`src/lib/directorios.ts`); una sin página diría «en camino» y no linkearía, pero **las cuatro existen: tres desde el 2026-09-11 y la de bibliotecas desde B-960**. Se llega por la pestaña «Guía» y, desde B-900, también por una fila del pie | Build Está en el sitemap desde el día uno, con sus filas todavía en camino: una fila que no linkea no promete un camino que no existe; `/proponer`, en cambio, estuvo afuera hasta que pudo recibir (B-896). Ver D-990. |
...
| `/guia/librerias/{slug}` | Las cuatro fichas de la Guía comparten el esqueleto `src/components/sitio/FichaDeGuia.astro` —migas, portada, galería, contactos y JSON-LD— y cada página le pasa sus campos (M-10). La ficha de una librería: dirección, provincia y barrio o ciudad (B-967), los cuatro contactos, la galería completa y el `BookStore`. **Cero JavaScript**, como el detalle — B-901 | Build (`caminosDeDirectorio('librerias')`) |
...
| `/guia/suscripciones` | El directorio de suscripciones literarias: todas las fichas publicadas en el HTML, más una island con los cuatro filtros del § 5 del PRD 3. **No estaba en este diseño** — B-832 | Build. La island baja `/suscripciones.json` |
| `/guia/suscripciones/{slug}` | La ficha: qué incluye, qué manda, extras, alcance, **el precio con su fecha de carga al lado** y la acción que nombra a quién le estás comprando. **Cero JavaScript** — B-832 | Build (`caminosDeDirectorio('suscripciones')`) |
| `/guia/suscripciones/sumar` | El formulario público de la sección. Lo propio: carga un **precio**, y su fecha la pone el servidor —quien carga no elige qué fecha se publica al lado del número (DEC-12)— B-832 | Build + island (`SumarSuscripcion`, `client:load`) |
| `/suscripciones.json` | El índice del directorio, aparte del `events.json` por lo mismo que el de librerías (B-832) | Build (ver [§3](#3-los-datos)) |
| `/guia/lugares` | El directorio de lugares para eventos: todas las fichas publicadas en el HTML, más una island con los cinco filtros del § 7 del PRD 4 —capacidad en rangos, costo en tres clases, **dónde queda** (la cascada provincia → barrio o ciudad, desde B-970), qué incluye y tipo—. Con esto la Guía queda completa: ninguna fila dice ya «en camino». **No estaba en este diseño** — B-833 | Build. La island baja `/lugares.json` |
| `/guia/lugares/{slug}` | La ficha: para cuántas personas, qué incluye, la condición con sus notas, el precio con su fecha al lado y los cuatro contactos. **La dirección sale solo si `direccionPublica`** (§ 6 del PRD 4): para una casa sale el barrio y una frase que dice que la dirección se pide escribiendo. **Cero JavaScript** — B-833 | Build (`caminosDeDirectorio('lugares')`) |
| `/guia/lugares/sumar` | El formulario público de la sección, y **la única página del sitio donde alguien puede tipear el domicilio de una casa**. Arranca con `direccionPublica` apagado y no ofrece la casilla; la regla lo exige (§ 6 del PRD 4) — B-915 | Build + island (`SumarLugar`, `client:load`) |
| `/lugares.json` | El índice del directorio, aparte del `events.json` por lo mismo que los otros dos (B-833) | Build (ver [§3](#3-los-datos)) |
| `/guia/bibliotecas` | El directorio de bibliotecas: todas las fichas publicadas en el HTML, más una island con la cascada de lugar, el tipo de biblioteca y un toggle de «hay que asociarse». **Es la cuarta y última sección de la Guía** — B-960 | Build. La island baja `/bibliotecas.json` |
| `/guia/bibliotecas/{slug}` | La ficha: los **dos** horarios —mostrador y sala de lectura—, el catálogo online, si hay que asociarse y cuánto sale (con su fecha de carga al lado), los cuatro contactos, la galería completa y el `Library`. **Cero JavaScript** — B-960 | Build (`caminosDeDirectorio('bibliotecas')`) |
...
| `/404.html` | La dirección que no existe: buscador, la tira de hubs y el enlace al archivo. **Cero JavaScript.** Firebase la sirve como cuerpo de cualquier ruta que no encuentre (B-310). Desde B-900 la tira termina con las secciones de la Guía **que tienen fichas publicadas** | Build. La tira sale de `exploracionDeLaHome` más `grupoDeLaGuia` (con los conteos de `fichasPorDirectorio`, en `src/lib/contenidoDeLaGuia.ts`); las frases, de `src/lib/noEncontrado.ts` |
...
| `/guia/suscripciones/{slug}` | `Product` + `Offer` + `BreadcrumbList`. **No es `BookStore` ni ningún `LocalBusiness`**: ese tipo exige `address` y una suscripción no tiene lugar, así que declararla así inventaría un local que no existe —y competiría con la ficha de la librería real que a veces la ofrece—. `Service` se evaluó: describe bien el club que no manda nada y no la caja que envía tres libros, y elegir por ficha daría dos marcados para la misma sección. **Y el `Offer` va sin `price`, a propósito** (§ 5 del PRD 3, DEC-12): Google lo muestra en el resultado de búsqueda, y un precio de tres meses se publica equivocado en el lugar de más visibilidad y con la credibilidad de un dato estructurado. El costo asumido es no ser elegible para el resultado enriquecido de precio (B-832) |
