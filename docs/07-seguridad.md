...
**Son veintiuna salidas, no dos.** Conviene tenerlas contadas antes de leer el resto,
...
> filas **20** y **21**, y la cuenta quedó en **veintiuna**.
...
| `modalidades[].inicio` / `modalidades[].fin` | **decisión, no olvido**: qué significa la ventana de una modalidad frente a las fechas de los encuentros sigue sin resolver (B-224), así que se guarda y no se publica en ninguna de las veintiuna salidas. Un campo que no sale no puede decir algo equivocado en el calendario de todos los suscriptos; agregarlo después es una línea | `toPublic.ts`, `calendario.js`, `textoRedes.ts`, `normalize.ts`, GA4 | **Y desde B-734 esa ausencia es además el motivo por el que el `subEvent` de una actividad con más de una forma de cursar no puede decir en cuál ocurre:** repartir los lugares de verdad necesitaría estos dos campos en `ModalidadPublica`, o sea revisar esta misma celda. Mientras tanto el `location` se hereda solo cuando hay una fila.
...
**`incluye` sí sale, y solo a dos de las veintiuna** (B-830, **D-580**). Va a la
