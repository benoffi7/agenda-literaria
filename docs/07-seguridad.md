...
**Enhanced Measurement de GA4 es una puerta que ninguna proyección de este repo cubre, y hay que cerrarla desde la consola de GA4, no desde acá.** Lo encontró el `auditor-privacidad` (D-253): `ubicacionSinQuery` y `via`/`eje`/`slug` gobiernan lo que **nuestro código** manda a mano, pero una vez que `gtag.js` está cargado, GA4 tiene un conjunto de eventos automáticos —activados por defecto en toda propiedad nueva— que leen la página **sin pasar por `construirEventoSitio`**:

- **«Búsquedas en el sitio»** lee parámetros de query con nombres típicos (`q` entre ellos) y manda el texto como `search_term` — es exactamente `?q=...` de `aQuery` (`listadoPublico.ts`), el mismo dato que `ubicacionSinQuery` recorta del `page_location` pero por una puerta distinta.
- **«Cambios de página según el historial del navegador»** dispara un `page_view` nuevo cada vez que la island de filtros llama a `replaceState` —o sea, en cada tecla de una búsqueda—, y ese `page_view` lee la URL **en el momento**, sin pasar por el `page_location` recortado del `config` inicial.
- **«Clics salientes»** manda el `link_url` completo de un link a otro dominio: el botón de inscripción linkea a `wa.me/<teléfono>` o a `instagram.com/<handle>`, que es exactamente el **destino** que `via` existe para no mandar.

**Ninguna de las tres se puede apagar desde el código de este repo.** Son ajustes de «Enhanced measurement» del flujo de datos, en la consola de GA4 — la misma clase de paso manual que ya existe en `docs/09-analitica.md` («qué tiene que hacer el dueño»), y acá es **bloqueante y no opcional**: hasta que alguien entre a GA4 → Administrar → Flujos de datos → el flujo → Enhanced measurement y apague «Búsquedas en el sitio» y «Clics salientes» (y, si la propiedad lo permite por separado, los `page_view` automáticos por historial), instalar el tag manda de más aunque todo el código de este repo esté bien. Está anotado como **B-480** en el `BACKLOG`, bloqueando el cierre real de B-372.
