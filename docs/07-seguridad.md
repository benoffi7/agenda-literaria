...
| 29 | El **correo semanal** (B-1230) — el borrador que se pega en Mailchimp y llega a la casilla de quien se anotó en la salida 13. **La segunda que deriva de otra salida** (la 1, como la 7 deriva de la 6) y **la primera que no se publica: se manda**, así que es la única que no se puede corregir con un rebuild — la clase de la 5, el texto para redes, con más alcance | `src/lib/boletinSemanal.ts` — `boletinSemanal` (qué encuentros entran y cómo se agrupan), `textoDelBoletin` y `htmlDelBoletin` (los dos cuerpos que se copian; el segundo escapa todo lo que viene de una actividad). **Su entrada es el `events.json` ya publicado y no el documento** (**D-801**): el link de la reunión, la dirección, las notas de difusión y los uids no están en la mano de este módulo, así que no hay nada que excluir. Arma cada fila con los **mismos** productores que el tríptico de la salida 1 —el lugar, el arancel, la etiqueta del tipo, la URL y todas las fechas— en vez de derivarlos otra vez (D-20): un correo con la hora corrida tres horas es la trampa 1 en su versión más cara. `src/components/admin/BoletinPanel.tsx` **solo acomoda** (D-140): es del panel, no del sitio, y no decide qué sale | `tests/boletin-semanal.test.ts`, `tests/barrido-de-salidas-publicas.test.ts` (el barrido del correo semanal, con su control negativo) |
...
(B-124, D-350). El correo no movió la cuenta.
