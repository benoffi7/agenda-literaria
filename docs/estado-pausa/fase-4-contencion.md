...
```
npx tsc --noEmit                  # 12 errores PREEXISTENTES de ImportMeta (B-173), ninguno mío
...
B-173 justamente porque hacen que el comando de verificación salga siempre en
...
| Ítem | Estado |
|---|---|
| **B-171 de la consigna = B-171 en el BACKLOG** · el detector apagado | ✅ **cerrado — volvió a `it`** |
...
> **Numeración:** la consigna llamó "B-171" al detector apagado, pero ese número
> ya estaba tomado en el BACKLOG (la versión sin estampar). Se renumeró a
> **B-171**, y el ítem lo dice en su primera línea.

## B-171 / B-171 — el estado, que es lo que más importa
...
| 7 | Query pública sin `where('estado','==','publicado')` | **— DESCUBIERTA — B-172** |
...
1. **B-172 (trampa 7).** En `tests/actividades.integracion.test.ts`, agregar dos
...
3. **B-173.** Meter un `astro sync` antes del `tsc` en
...
- **B-172** · la trampa 7 del §13 no tiene ningún test · P2 — toca
...
```
Fase 4: estado de pausa con el diagnostico de B-171
B-171: el detector de triggers blindados sigue la llamada, y vuelve a estar prendido
