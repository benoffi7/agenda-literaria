...
| Que un test de integración nuevo arme su **propia** credencial del emulador en vez de pasar por el helper | `credenciales-del-emulador.test.ts` (**B-1060**): barre `tests/` buscando llamadas a `createCustomToken` o `setCustomUserClaims` fuera de `tests/fixtures/credenciales-del-emulador.ts`, con dos excepciones escritas a mano y con su motivo. La clase que cierra no es el tipeo: catorce copias casi iguales podían diferir en algo que importa sin que nadie lo viera, y de hecho lo hicieron — B-895 unificó los claims hacia la vía de producción en diez archivos y se salteó `reportes-resuelto.integracion.test.ts`, que los pasaba por las **dos** vías a la vez y habría seguido verde aunque la vía fiel dejara de funcionar. La diferencia que sí es deliberada vive como opción con nombre (`email`/`emailVerificado`, porque el correo va en el **registro de la cuenta** y nunca en los claims — B-888) y no como un cuerpo aparte. Hubo una segunda, `claimsEnElToken`, que embebía los claims dentro del custom token; **se eliminó el mismo día**, cuando B-1030 —el hallazgo que la sostenía— resultó ser un artefacto de worktree. Probado por mutación en las dos direcciones: una copia sintética lo pone rojo, y **nombrar** las dos funciones en un docblock —como hace `guardas-de-los-scripts.test.ts`— no alcanza para ser infractor, que es lo que evita una lista de excepciones inflada |
| Que el emulador de Auth sirva **otro proyecto** que el del checkout | `proyecto-de-auth.test.ts` (**B-1112**, D-730): el emulador de Auth es de un solo proyecto —el de su `--project` de arranque— y no lo publica por ninguna vía consultable, así que `entrarComo` mira el `aud` del ID token en el primer login del proceso y falla nombrando los dos proyectos. La clase que cierra no es el desajuste sino **cómo se manifiesta**: un `PERMISSION_DENIED` sobre un documento válido, y **asimétrico** —un uid que ya existe en el store del emulador pasa y uno nuevo no, con el mismo código—, que es lo que hizo cerrar B-1021 y B-1030 con la causa equivocada. El test cubre la mitad pura (`desajusteDeProyecto`); la que no se puede probar sin un emulador mal levantado es la llamada. **Y quedan escritas en `tests/emulador.ts` las dos salidas que no sirven, medidas y no argumentadas:** el endpoint de config contesta `200` para cualquier proyecto, y `"singleProjectMode": false` solo apaga el aviso. Degrada a «no disparar» con cualquier token raro, a propósito: corre en el login de todos los tests de integración y un falso positivo acá se parecería al bug que nombra |
| Que un script se escriba su **propia** normalización de ciudad en vez de importar el `slugify` del proyecto | `ciudades.test.ts` (B-919, D-690): barre `src/`, `functions/` y `scripts/` buscando `normalize('NFD')` —la firma inconfundible de esta normalización— y exige que viva solo en los dos módulos que la tienen permitida (`slugify.mjs`, que produce identificadores, y `normalize.ts`, que produce el índice de búsqueda del §6: son dos preguntas distintas). Es un chequeo de **clase**: el script que se la copie entra en rojo el día que se escribe. Hace falta porque el claim lo escribe node y el documento lo escribe el panel, y si los dos normalizaran distinto el permiso no matchearía y el síntoma sería «no hay actividades de tu ciudad» |
| Que las reglas rechacen lo anónimo y lo que no es admin | `actividades.integracion.test.ts` y `reportes.integracion.test.ts`, con `EXIGIR_EMULADOR=1` en CI para que no se salteen en silencio. **Esta fila decía menos de lo que parecía hasta el 2026-08-27:** los casos anónimos eran de **escritura**, y del lado de la lectura había un `it('un anónimo lee lo publicado')` en verde que estaba fijando una fuga (B-208, D-128). Hoy hay tres `it` de lectura —por documento, por query, y un control positivo— y el archivo empuja las reglas del checkout al emulador con `cargarReglas`, que antes no hacía. **Y desde B-836 hay una tercera mitad, que es la que faltaba:** los dos archivos de arriba cubren *su* colección, así que la colección nueva nacía sin barrido. `escritura-anonima.integracion.test.ts` afirma la propiedad **del proyecto** —hoy nadie escribe sin el claim, ni anónimo ni logueado sin claim— sobre una lista de colecciones **sacada de `firestore.rules`**, así que un `match` nuevo entra solo; más las cuatro colecciones que los PRDs iban a crear y una ruta inventada, que es lo que prueba la red del `match /{document=**}`. **La lista de excepciones (`COLECCIONES_ABIERTAS`) ya no está vacía: hoy tiene cuatro** —`propuestas` (B-896, 2026-09-11), `librerias`, `suscripciones` y `lugares` (B-831/B-832/B-915, 2026-09-15)—, y el criterio que esta fila defiende es el que sigue valiendo: **abrir una es un cambio visible en un test**, escrito a mano y con su motivo al lado, no un efecto colateral de tocar las reglas. Hasta el 2026-09-15 esta celda decía «está vacía» como evidencia, y esa mitad era vieja desde B-896 (lo cobró el `auditor-documentacion`). ⚠️ **Y una limitación que la fila no decía:** este archivo es testigo de la **lista**, no de la forma — prueba con un documento sonda que cualquier `hasOnly` rechaza con la puerta abierta o cerrada, así que el control positivo de cada puerta vive en el archivo de su colección. Verificado por mutación: un `allow create: if true` en una colección **cerrada** lo pone en rojo, y la colección entra al barrido sola |
...
| Que ningún campo privado del modelo llegue al `events.json`, al evento de Calendar, **a la página de detalle ni a la cartelera** cuando se agrega un campo nuevo | `barrido-de-salidas-publicas.test.ts` (B-196, extendido en B-227, B-265 y B-1230): inyecta centinelas y los busca en cada salida que proyecta —son **seis**: el `events.json`, el índice, el evento, la página de detalle con su JSON-LD, la cartelera y, desde B-1230, **el correo semanal** (su estructura y sus dos cuerpos, con control negativo)—, en las dos direcciones, con un fixture que se autoexige actualizado campo por interfaz. **Le sacó trabajo al `auditor-privacidad`**, que ya no tiene que reportar la instancia — solo el campo nuevo que el fixture todavía no ancló |
...
| Que el **panel** atenúe texto por debajo de WCAG AA —el piso es `text-tinta/65`— | `contraste-del-panel.test.ts` (**B-1630**). Los dos de arriba barren solo el sitio; éste barre `components/admin/` y `components/campos/` con el mismo método: colores de `global.css`, superficies del markup (el blanco, el papel del fondo y cada token que el panel use como fondo pleno) y cada `text-tinta/NN` medido contra la más oscura. Control positivo, control negativo (`/60` pasa sobre blanco y no sobre papel, que es por lo que no se mide solo contra la tarjeta) y una lista `EXCEPCIONES` donde cada entrada lleva su porqué y **falla si queda huérfana**. Exime `disabled:` (WCAG 1.4.3). Mide también sobre los tintes del markup (paleta de Tailwind instalada, compuestos sobre la base más oscura) y cada par fondo + tinta del mismo grupo de clases (**B-1751**); frena un `opacity-NN` que atenúa, con `OPACIDADES_CON_MOTIVO` y `FONDOS_SIN_TEXTO_ATENUADO` que fallan si quedan huérfanas (**B-1750**, D-1105). Recorre el árbol JSX de cada archivo para la tinta sobre el tinte de un ancestro, y `contraste-del-arbol.render.test.tsx` lo mide sobre el DOM en los `Aviso*.tsx`, con un registro que falla si un aviso no se monta (**B-1830**, D-1125). No ve el tinte de otro componente fuera de los avisos (B-1870) |
...
| Que un campo de formulario quede **sin nombre accesible** — el `<label>` pintado al lado de un control que no lo referencia | `clases-de-bug.test.ts`, el `describe` de **B-827**. La garantía existía y era **opt-in**: `Campo` aceptaba un `htmlFor` opcional y once usos no lo pasaban, así que un lector de pantalla anunciaba «cuadro de texto» y nada más en un formulario de treinta y pico de campos. Ahora `htmlFor` es requerido —eso lo cobra el compilador, que es quien enumeró los 37 usos que faltaban— y este barrido cubre **la mitad que el tipo no puede ver**: que el hijo lleve *ese mismo* id, porque un `htmlFor` apuntando a un id que nadie declara typecheckea igual y deja el label tan huérfano como antes. El campo que rotula un grupo de controles se declara `comoGrupo` y pasa a `role="group"` + `aria-labelledby`, que es el mecanismo que corresponde. Y hay un caso de render (`formulario-apilado.render.test.tsx`) que busca «Título» **por label**: el barrido lee el fuente y no puede saber si el id llega pintado. Es la clase de B-821 —una garantía voluntaria— y ahora importa más, porque `campos/` salió de `admin/` para que lo usen los formularios públicos de `prd/`. **Adentro** de un control con dos ramas excluyentes el barrido no llega —`TaxonomiaSelect` tenía el id solo en el `<select>` y no en el input de «Otro», que además hace `autoFocus`—: eso lo cubre `taxonomia.test.ts`, que ya era la guardia de ese par de widgets |
| Que un **dato que envejece** —el precio de una suscripción, las promos de una librería— se publique sin la fecha en que se cargó, o entre a un filtro, a un orden o a un `Offer` | `dato-con-fecha.test.ts` (**B-837**, D-570). Pero la parte que importa no la hace el test: la hace **la forma**. `src/lib/datoConFecha.ts` no exporta ninguna función que devuelva el valor solo —la única salida es la frase, un string como «$18.000 por mes · cargado el 24 de septiembre de 2026»—, así que no hay número que filtrar y no hay manera de mostrar el valor sin su fecha. El test fija las tres reglas de los PRDs y, sobre todo, **la vuelta**: con el `cargadoEn` ausente, en `null` o como string, el valor **no sale** (y no tira), y el dato huérfano pide revisión en el panel para que no quede invisible en los dos lados. Un auditor acá sería redundante hoy y quedaría desactualizado el día que aparezca el primer consumidor |
| Que App Check quede cableado con el **proveedor equivocado**, o que alguien agregue a mano el `<script>` de reCAPTCHA que la consola de Google muestra | `appcheck.test.ts` (**B-836a**). Los dos errores son de la misma clase —se ven razonables y no fallan hoy—: con `ReCaptchaV3Provider` sobre una clave **Enterprise** el token se rechaza, y eso **no se nota hasta que el enforcement está activo**, o sea el peor momento; y la integración genérica de reCAPTCHA —cargar `recaptcha/enterprise.js` y llamar `grecaptcha.enterprise.execute()`— está en la documentación de Google, así que se agrega de buena fe y se pelea con el SDK por el mismo widget, con un síntoma que no es un error sino un comportamiento raro. Cubre además **la posición** de la llamada (adentro de `app()` y antes del `return`, que es lo que garantiza que App Check esté inicializado antes de la primera petición) y que el import siga siendo estático. Los asertos de prohibición corren sobre el fuente **sin comentarios**, porque los docblocks de `appcheck.ts` nombran justamente lo que prohíben |
| Que un componente **reciba** el `id` de su `Campo` y no lo reenvíe a su control real | `campo-asociado.render.test.tsx` (**B-827**), y existe porque es **el punto ciego del barrido de texto** de la fila de arriba: ése exige que dentro de `<Campo htmlFor={X}>` aparezca `id={X}`, y con eso no puede ver que el hijo se quede el prop — el fuente dice `id={campoId('x')}` y el `<input>` sale sin nada. Monta el formulario de verdad y busca **por label** (`getByLabelText`, que resuelve la asociación sobre el DOM), uno por tipo de control: input suelto, textarea, select, `TaxonomiaSelect` y `TagsInput`; más el caso del grupo, que se busca distinto (`getByRole('group', { name })`), y un control positivo que distingue «el label se rompió» de «no hay formulario». Lo pidió el `auditor-trampas` sobre la tanda de B-830, y es la única forma de cerrarlo: un test que lee el fuente no puede saber si el id llega pintado. Verificado por mutación — sacar el `id={id}` de `TagsInput` lo pone en rojo y deja el barrido de texto en verde |
| Que una **página del sitio público** alcance la medición del **panel**, o la cadena que ésta arrastra | `panel-fuera-del-sitio.test.ts` (**B-841**). Recorre el grafo de imports desde **cada** página de `src/pages` —siguiendo también los `import()`, porque para esto un diferido mide igual— y exige que ninguna salvo `/admin` alcance `@/lib/analytics`, `@/lib/firebase-client` ni `@/lib/appcheck`. El riesgo no es de capas: la analítica del **panel** no tiene portón de consentimiento —nunca lo necesitó— así que un `Seccion` en una página pública mediría y crearía el perfil en `localStorage` para alguien que no tocó el banner, en la misma propiedad de GA4 (B-801), contra D-250 y la promesa de `/apoyar`; y desde B-836a la cadena llega a `firebase/app-check`, o sea un tercero de Google con cuota facturable por visitante. **Ninguna de las redes que ya existían lo veía**: `terceros-antes-del-consentimiento.test.ts` lee el HTML de `dist/` buscando `<script src>` absolutos y los dos SDK se inyectan en runtime desde un chunk local, y `bundle-panel.test.ts` mira el chunk inicial **del panel**, que es la dirección contraria. Con control positivo (`/admin` **sí** la alcanza) y verificado por mutación. Lo pidió el `auditor-privacidad`. **Y desde que B-841 está hecho tiene una segunda mitad**, porque las dos miran la misma propiedad desde lugares distintos: aquélla desde **las páginas**, ésta desde **el directorio** —ningún archivo de `components/campos/` alcanza `@/lib/analytics`, `firebase-client`, `appcheck`, `lib/opciones` ni `firebase/firestore`—. La primera se pone roja cuando alguien **ya escribió** el formulario público que lo arrastra, o sea tarde; la segunda cuando alguien mete el import en `campos/`, que es donde el error se comete. Con control positivo (la capa `campos-del-panel.tsx` **sí** las alcanza), el resolvedor de los alias a `functions/` con su propio control —`campos/TaxonomiaSelect.tsx` importa de `@calendario`, y sin resolverlo el recorrido devolvía «no hay camino» por no haber sabido mirar— y las dos verificadas por mutación |
| Que la capa que ata los controles compartidos al panel **deje de atarlos**, y la medición se apague sin que nada falle | `campos-del-panel.render.test.tsx` (**B-841**). Es la clase que el propio refactor crea: la medición y la ayuda pasaron a props **opcionales**, así que no pasarlas typecheckea igual y se ve idéntico — lo que se pierde es GA4 sin `funcion_usada` para todas las aperturas de sección y toda interacción de taxonomía del panel, y se nota semanas después mirando un hueco en el tablero. Monta la capa y afirma **la llamada**, no el texto; y la otra dirección, que es la razón de ser del corte: el control **genérico** no mide nada sin la prop. Lo pidió el `auditor-trampas`, que además notó que el refactor **degradó** un aserto existente —«TagsInput mide la taxonomía» pasó a probar que el literal existe y no que se mida, porque ahora es el argumento de una llamada que puede no tener a quién llamar—; ese caso se renombró a lo que de verdad prueba |
| Que el borrado de la foto de un tercero se decida mal, o que una rama nueva se saltee la verificación sin decirlo | `propuestas-imagen.test.ts` (**B-863**, **B-926**). Es puro y mira tres cosas que no se pueden mirar con un `grep`: la **decisión** (`decidirBorradoDeImagen`, con sus siete caminos y el que agregó el descarte), la **verificación** (`borrarOriginalAlAceptar`, que se niega a borrar sin una copia en el documento **y** en el bucket) y la **forma del trigger** — que el borrado crudo no sea alcanzable desde la rama de la aceptación, que el `else` final no borre por default, y que los cinco caminos que dejan la foto viva lleven `alerta` para poder filtrarlos (B-871). Los cortes sobre el fuente van acotados por índices reales y no hasta el final del archivo: un `slice` abierto haría que el chequeo mirara código de otra sección y pasara por motivos ajenos. **Verificado por mutación** en cada uno de esos tres frentes |
| Que la forma de una propuesta se valide **solo en el cliente**, o que los topes de `firestore.rules` y del modelo se separen | `propuestas.integracion.test.ts` (**57** casos contra el emulador — eran 33 hasta B-926, que sumó los del flag `fotoDescartada`) + `propuestas.test.ts` (**B-830**). Es el par que más importa de los que hay: del otro lado de `/propuestas` va a haber un **anónimo**, así que el schema de zod no es la defensa —se saltea con un `curl`— y la defensa es `propuestaValida()`. El de integración prueba **contra el emulador** porque un `grep` al archivo pasaría con la regla escrita mal (`!=`, el campo renombrado), y el puro ata **las veinticuatro cotas** del bloque y los cuatro vocabularios leyendo `firestore.rules`, que es el único modo de atar un runtime que no puede importar TypeScript (el patrón de B-364). La comparación es **exhaustiva y no una muestra**: el test parsea todas las cotas del archivo y las compara contra la lista completa de constantes, así que una cota nueva que nadie declare lo pone en rojo — la primera versión ataba ocho de veinticuatro y `TOPE_CORTO_PROPUESTA` quedaba suelto en cuatro de sus cinco ocurrencias, que es el mismo patrón cumplido a medias. Y el recorte del bloque se ancla al **comentario de sección**, no al nombre del primer helper, para que un helper agregado antes no quede afuera del barrido. **Verificado por mutación, y ahí ganó su valor**: encontró dos cláusulas mías que no hacían nada —un `size() == 1` y un `hasAll`, las dos con un comentario que decía que eran lo que frenaba— y un aserto que afirmaba que la regla validaba la forma de cada fecha, que es falso porque una regla no itera listas (**B-842**, anotado como asimetría y no como garantía). Fija además el estado de la puerta: el `create` anónimo está cerrado hasta que App Check exija, y el día que se abra estos casos se ponen rojos |
| Que la conversión de una propuesta a actividad publique un `incluye` que no está en la taxonomía, o que la aritmética de fechas vuelva al reloj del proceso | `propuestas-conversion.test.ts` (**B-830**, D-590, D-600). El primero es un hallazgo del `auditor-privacidad` y no una precaución: `actividadFormSchema` declara `incluye` como `z.array(texto)`, así que **no filtra nada**, y el camino sigue —`toPublic` lo proyecta, `detallePublico` lo resuelve, `listadoPublico` cae a `desSlug(valor)`— o sea que un slug inventado **se publica** des-slugueado en la página de detalle, que es HTML indexado. Y es el campo donde «el admin lo va a ver» es **más débil**: doce chips que parecen taxonomía se leen como taxonomía. El segundo tiene una particularidad que conviene saber: la aritmética va sobre `Date.UTC` y el motivo es el **DST**, no la zona del proceso —la versión con reloj local pasa los veinte casos en Buenos Aires y en Tokio, lo mostró la mutación, porque el parseo y los getters locales se cancelan—, y como la suite no puede cambiar la zona caso por caso, lo que tiene red es **la forma**: un aserto sobre el fuente prohíbe los constructores de reloj local. Cubre además lo que **no** viaja, que es donde una ausencia se lee como olvido: el contacto, el estado, el slug y el canal de inscripción |
| Que el contacto de un tercero **sin login** llegue armado a un `href` del panel, o que la segunda escritura de D-600 se dispare desde el botón en vez de desde el guardado | `bandeja-de-propuestas.test.ts` y `propuestas-panel.render.test.tsx` (**B-830**, paso 7). El primero es una superficie que el proyecto no tenía: la bandeja muestra texto escrito por alguien sin cuenta, y un `href` es donde un string ajeno deja de ser texto —`javascript:` en el panel de un admin logueado es código con su sesión, y React escapa el **contenido** de un atributo, no su esquema—. `enlaceDeContacto` arma cada vía por separado y **nunca** concatena el valor crudo; cinco venenos (`javascript:`, con mayúsculas y con espacio inicial, `data:`, `vbscript:`) por las tres vías, verificados por mutación. Y el render test es el que fija que la pantalla **la use**: el caso del link que se arma y el del que no, porque interpolar el valor crudo pone rojos los dos y ninguno solo alcanza. El segundo lee el fuente de `AdminApp.tsx` —mismo patrón y mismo motivo que `salida-del-panel.test.ts`— porque la propiedad no vive entera en ningún componente: la bandeja arma el `alGuardar` y no lo llama, el chasis lo llama y no sabe qué hace. Si se disparara desde `onConvertir`, una conversión abandonada dejaría la propuesta `aceptada` apuntando a una actividad que no existe, y la bandeja ya no la muestra. Cubre además que la pantalla **no ofrezca** editar el contenido de una propuesta (§4.3 del PRD) y que `propuesta-rechazada` no se mida cuando la escritura falló |
| Que **el plazo que la bandeja promete** y **el que el barrido aplica** se separen | el `describe` «la bandeja y el barrido dicen lo mismo» de `bandeja-de-propuestas.test.ts` (**B-844**). Es el patrón de B-364 —dos runtimes atados por un test en vez de por un import— con una vuelta más, y hace falta porque acá el import **sí era posible** y no se hizo: un cuarto alias a `functions/` toca seis archivos compartidos (`astro.config.mjs`, `tsconfig.json`, `vitest.config.ts`, `scripts/que-deployar.sh` y las listas de alias de `bundle-panel.test.ts` y `panel-fuera-del-sitio.test.ts`), así que `caducaEn` (`src/lib/bandejaDePropuestas.ts`) es **una segunda implementación declarada como tal** de `decidirRetencion` (`functions/retencion.js`). En vez de comparar dos números —que dejaría suelto el reloj y la tabla de estados—, pasa **una familia de catorce fixtures por las dos** y exige la propiedad que le importa a quien mira la pantalla: la ficha nunca dice «todavía falta» de algo que el barrido de esta noche se lleva, ni «se borra» de algo que conserva. Con control positivo (si todos los fixtures cayeran del mismo lado, el caso pasaría sin comparar nada) y un aserto directo de los plazos en milisegundos, que dice **cuál** de las dos se movió. El borde exacto del plazo lo pidió el `auditor-trampas` y es el único aserto de la suite que agarra un off-by-one de un lado solo (`<` contra `<=` del lado que borra). **Que el import siga siendo mejor está anotado**, en el docblock de `bandejaDePropuestas.ts` |
| Que el barrido de retención vuelva a borrar **sin precondición**, o que la relectura que protege la imagen se caiga debajo del borrado del objeto | `retencion.test.ts` (el `describe` del orden) + `retencion.integracion.test.ts` (**B-864**). Son tres posiciones en el fuente —relectura, objeto, documento— y **ninguna la fijaba nada**: quitar el `{ lastUpdateTime }` o bajar el `getAll` deja toda la suite en verde, porque la carrera es de segundos por día y ningún test la produce sola. El caso de integración la **arma explícitamente** —lee, escribe a mano lo que el admin habría escrito, y recién entonces borra con la lista vieja— y afirma **las dos mitades**: el documento y su imagen. Las tres mutaciones dicen exactamente qué cubre cada guarda: sin precondición se pierden las dos, sin relectura se pierde **solo la imagen** (que es la tensión con el orden de B-838), y sin `{ lastUpdateTime }` se pierde el documento en la ventana que la relectura no alcanza. Hay además un aserto de que la relectura sea `getAll(ref, { fieldMask: [] })` y **no** un `ref.get()`, porque un `get()` traería el contacto del tercero a la memoria de la Function y desharía la garantía de B-838 sin que nada más se moviera. **La primera versión del aserto de orden tenía un falso verde que encontró una mutación**: `bucket.file(objeto).delete` aparece dos veces y el `indexOf` pelado miraba la de la rama temprana, así que la inversión del orden pasaba — ahora se busca desde la guarda de la versión |
| Que el barrido de flyers de `propuestas/` se lleve el flyer de una propuesta **abierta**, el de una aceptada **antes** de los 30 días o sin fecha legible, o un objeto de la galería | `retencion.test.ts` (el `describe` de `decidirFlyeresSinPlazo` y el de `borrarFlyer`) + `retencion.integracion.test.ts` (**B-871**, D-1160/D-1161). La decisión borra solo `aceptada-vencida` y `sin-propuesta`, y los casos fijan cada frontera: el flyer de una `nueva`/`en-revision`/`rechazada` sigue siendo de la retención **aunque su documento ya venció**, un día antes de los 30 no se borra, sin `revision.en` legible no se borra (y no cae a `creadoEn`), dos documentos sobre el mismo objeto no se resuelven, y lo que no es `propuestas/<un segmento>` no llega nunca a `aBorrar` — `borrarFlyer` además tira si se lo piden. El de integración arma las dos carreras que la relectura cubre —la propuesta que **reabren** en el medio de la corrida y el envío de `/proponer` que llega justo para su huérfano— y afirma que el documento de la aceptada **sobrevive** a su foto. Un aserto sobre el fuente ata que la Function y el script usen **la misma** decisión y el mismo borrado |
...
| Que la proyección de una **librería** filtre el contacto de quien la cargó, o que un campo nuevo del modelo salga sin que nadie lo decida | `libreria-publica.test.ts` con su fixture `tests/fixtures/centinelas-libreria.ts` (**B-901**). Es la forma de B-196 aplicada a la primera colección del proyecto que **no es una actividad**, y hace falta un fixture propio porque el barrido canónico recorre `RUTAS_CENTINELA`, que son campos de `Actividad`. Lo que esta salida tiene y ninguna otra: el documento lleva el **segundo dato personal de un tercero** (`contactoDeQuienCargo`) conviviendo con **cuatro contactos que sí son públicos** —el punto de la ficha es que la gente le escriba a la librería—, que es la condición exacta donde un spread filtra un campo. Tres listas de permitidos separadas y justificadas —la proyección, la ficha y el marcado estructurado—, las dos direcciones (ninguno de más y ninguno de menos), y el **control negativo codificado** de B-212: se arma la proyección con un spread y se exige que el barrido falle **nombrando** el campo, porque sin eso el caso podría estar verde por no mirar nada. Más la cobertura del fixture contra el fuente de `src/types/libreria.ts`: un campo nuevo que no tenga centinela ni declaración deja el archivo en rojo, así que no puede entrar al barrido con un valor inocente |
| Que el build del directorio lea lo que **no** va a publicar, o que la ficha nazca sin su marcado y sin su entrada de sitemap | el **paso 8i** de `scripts/build-contra-emulador.mjs` (**B-901**, **B-903**), sobre el `dist/` de verdad. Siembra **dos** librerías —una publicada y una esperando decisión— porque con una sola un build que leyera la colección entera daría exactamente el mismo artefacto, y afirma que la pendiente no está en `dist/librerias.json`, no tiene página y no está en el sitemap. Es la mitad que ningún unitario puede ver: `tests/librerias.test.ts` lee el fuente y comprueba que la query lleve el `where` con `ESTADO_PUBLICO` y el `.select()` de D-159 —o sea que ni el contacto interno entra a la memoria del build—, y esto comprueba que el build de verdad lo haya respetado. La mutación está probada en las dos capas: sacar el `where` pone en rojo el test de fuente **y** cuatro asertos del gate, incluido el barrido de centinelas sobre todo el `dist/`. **La fila decía «8i» y hoy son cuatro pasos**, uno por colección de la Guía: **8j** suscripciones (**B-832**), **8k** lugares (**B-833**, con la casa publicada que prueba una ausencia condicional) y **8l** bibliotecas (**B-960**). El 8l entró en el pase de auditores y no con su frente, y esa demora es el motivo por el que esta fila ahora los nombra a los cuatro: sin su paso, el barrido del paso 9 recorría `dist/bibliotecas.json` y `dist/guia/bibliotecas/**` **sin un solo centinela que pudiera encontrar** — verde por vacuidad, la clase de B-873. Cada colección nueva de la Guía necesita las cinco piezas: sus centinelas, su canasta, su rama en el selector, el par publicada/pendiente sembrado y su paso |
| Que se publique una librería o una suscripción y el sitio estático **no la muestre nunca**, que cada publicación cueste dos builds, o que las dos colecciones compartan una lista y un campo propio de una no dispare build nunca | `directorios-rebuild.test.ts` (**B-901**, **B-832**, trampa 8). `functions/` no puede importar de `src/` (D-20), así que `CAMPOS_PUBLICOS_POR_DIRECTORIO` es una **segunda escritura por colección** de lo que publica cada proyección: el test recorre una tabla (`it.each`) y compara cada una contra la suya, y la mutación —sacar `'barrio'` de librerías, `'periodicidad'` de suscripciones— deja en rojo el caso nombrando la colección. El efecto real de esa divergencia es mudo en las dos direcciones: un campo de menos y corregirlo en una ficha publicada no llega nunca al sitio; un campo de más y el write-back de un trigger dispara un build por sí solo. **Desde B-832 hay un tercer caso, y es el que la lista compartida hacía imposible ver**: que cada colección tenga su propia lista y su campo propio, porque un campo que existe en una y no en la otra se comparaba contra `undefined` en los dos lados y **nunca cambiaba**. Cubre también la guarda contra el write-back de `publicadaAlgunaVez` —el trigger que falta (B-905)—, que corregir el **contacto interno** no cueste un build, y que la colección que nadie declaró rebuildee (la dirección barata del error) |
| Que la proyección de una **suscripción literaria** filtre el contacto de quien la cargó, o que el **precio** se publique sin su fecha | `suscripcion-publica.test.ts` con su fixture `tests/fixtures/centinelas-suscripcion.ts` (**B-832**). Lo primero es la forma de B-901 una colección más abajo, con tres listas de permitidos, las dos direcciones y el control negativo codificado. Lo segundo **no se puede verificar con un centinela**, y esa es la parte interesante: lo que sale de `precio` no es un texto del documento sino **una frase armada** (`$18.000 por mes · cargado el 24 de septiembre de 2026`), así que un centinela no podría distinguir «se publicó la frase» de «se publicó el número solo» — que es justamente el modo de falla. Se verifica **por valor**: sin fecha usable no sale nada, sin período nombrable tampoco, el monto no aparece por ninguna otra puerta de la proyección, y `TEXTO_POR_PERIODO` está atado contra `opciones-base.json` para que un período nuevo no deje un precio que nunca se publica. El caso que fija que **el precio no entra al `Offer`** (criterio 5 del PRD, D-670) está escrito para leerse cuando falle: dice por qué está y adónde ir si de verdad hay que revisar la decisión |
| Que el **monto** de una suscripción quede publicado sin su fecha en alguna pantalla | el **paso 8j** de `scripts/build-contra-emulador.mjs` (**B-832**), y es lo que ningún unitario puede ver. La garantía de D-570 la da la forma —`fraseDePrecio` devuelve **un** string— y los unitarios la verifican sobre la función pura; el gate la verifica sobre **lo que quedó escrito en el `dist/`**: cada aparición del monto sembrado en cualquier archivo publicado tiene que traer «cargado el» pegado, más el control positivo de que aparezca en alguna parte. Es el modo de falla que una función pura no puede impedir: que alguien, en una plantilla o en una island, arme la frase por su cuenta y pinte el número solo «porque en la tarjeta angosta no entra la fecha». El mismo paso siembra **dos** suscripciones —una publicada y una esperando decisión— con el argumento de 8i, y afirma que el JSON-LD sea `Product` sin `price` y que la ficha enlace la librería publicada que la ofrece |
...
| Que el panel pierda el **aviso de foto rotada**, o que alguien invierta el orden de lectura del EXIF respecto de `sinMetadatos` | `imagenes-archivo.test.ts` (**B-324**). El parseo de `Orientation` se verifica **ejecutando** —los dos órdenes de bytes, un IFD que apunta afuera, `null` para todo lo que no entiende—, pero las tres cosas que hacen que el aviso exista se afirman **sobre el fuente**, porque `subirImagen` habla con Storage y ningún test lo ejecuta: que la orientación se lea del crudo **antes** de `sinMetadatos` (invertirlo da `null` siempre y el aviso desaparece sin que nada se ponga rojo), que lo que se sube sea `limpio` y **nunca** `crudo` —lo pidió el `auditor-privacidad`: son dos arrays vivos en la misma función y uno lleva el GPS adentro—, y que el `1` se convierta en `null`, que es lo único que impide que cada foto derecha avise. Cubre además que el aviso use `role="status"` y no la tinta del error. **Y una mutación desmintió el motivo que el caso de los offsets iba a declarar**: sin las dos guardas de rango el parser sigue verde, porque un índice fuera de rango en un `Uint8Array` da `undefined` y los operadores de bits lo hacen 0 — la seguridad ante un archivo roto no viene de las guardas, viene de cómo se leen los bytes, y el día que esto pase a `DataView` el caso se pone en rojo |
| Que la lista de segmentos JPEG del saneador vuelva a ser **negra**, o que se quede con el perfil ICC | `imagenes-archivo.test.ts`, `describe('la lista de segmentos JPEG es blanca — B-869 / D-620')`, más la atadura de `imagenes-function.test.ts`. Es la clase que B-323 cerró para PNG y dejó **explícitamente abierta** para JPEG: una lista negra deja pasar todo lo que no nombra, y acá lo que no nombraba —el APP11 de C2PA— es justo lo que `quedanMetadatos` sí busca desde B-220, así que el saneador se quedaba corto donde el detector miraba y la foto se **rechazaba** con un cartel que le echaba la culpa al teléfono; el APP2 del índice MPF, que el detector no busca, directamente **se subía**. Los casos fijan las dos mitades de la regla —los APPn se tiran salvo firma conocida, y lo estructural (DQT, DHT, DRI, SOF, SOS) **no pasa por la lista**— más los dos pares que obligan a que la lista sea por firma y no por marcador: `ICC_PROFILE\0` se queda y `MPF\0` se va aunque compartan el `0xE2`; `JFIF\0` se queda y `JFXX` se va aunque compartan el `0xE0`. El caso del APPn inventado (`0xE7`) fija la propiedad de fondo, calcado del `'zzZZ'` de B-323: con la lista blanca se tira **lo haya visto alguien antes o no**. **Y dos casos más los pidió el `auditor-privacidad` sobre la primera versión del arreglo**, porque los dos desmentían lo que la decisión afirmaba: que un APP0/JFIF **con thumbnail embebido** no se conserve —la firma sola lo dejaba pasar, y ahí adentro va una miniatura del original de antes del recorte, la misma clase que `JFXX`—, con su par de control; y que un marcador **reservado** (`0xF7`) tampoco se conserve, que es lo que obligó a mover el corte de «APPn o COM» a «lo que no está en la tabla estructural del spec». **Y la re-auditoría agregó tres más**, todos de la misma forma —la regla decía más de lo que el código chequeaba—: que `0xC8` (`JPG`, reservado, dentro del rango `0xC*`) se tire como su gemelo `0xF7`; que un JFIF o un Adobe con **cola pegada** detrás de su forma fija no se conserven, porque reconocer la firma no acota el cuerpo; y que los estructurales **sin cuerpo** (RSTn, TEM) sobrevivan, que era lo único del conjunto que ningún caso ejercitaba —sacar `0xD0` dejaba la suite entera en verde—. Y el aserto de `imagenes-function.test.ts` cierra la clase de B-88 ejecutándola —lo que el panel sube, la Function lo entiende— porque con dos tablas separadas cada imagen se recomprimiría de más en silencio. Quince mutaciones probadas, una por aserto |
| Que el tag del deploy se arme a mano en el YAML y se separe de lo que muestra el panel | `workflows.test.ts` (**D-510**, clase de B-88). El tag de cada push es *exactamente* la cadena que el panel muestra y que un reporte de bug copia, así que tiene que salir de `scripts/version.mjs` y no de un `echo` con `git rev-parse`: armada dos veces, el día que cambie el formato el tag y el panel dirían cosas distintas y `git show` de lo que reporta una persona no encontraría nada. El chequeo mira el job `etiquetar` y exige los **dos** tags —el de deploy y el de versión—, la guarda de árbol sucio y que un re-run del mismo commit no falle |
| Que una puerta de carga diferida nueva del panel quede sin límite de error, y un chunk borrado deje **el panel en blanco** | `carga-diferida.test.ts` (**B-805**). Es de clase: recorre `src/components/admin/**` buscando `lazy(` y exige que cada archivo que declare uno lo envuelva con `SiNoCarga`, y que el límite quede **por encima** del `Suspense` —que no atrapa errores—. Nació de un P1 que los dos auditores encontraron sobre el propio arreglo: la cuenta decía «seis vistas» y el `lazy` del centro de ayuda, que se monta en el encabezado, quedaba afuera de todo límite. Cubre además que el aviso **no** prometa el borrador donde no hay autoguardado |
| Que un campo nuevo de `Actividad` quede sin nombre de pantalla en el historial y se muestre con la clave cruda | `tests/historial-actividad.render.test.tsx` (**B-181**). Es de clase: lee los campos de primer nivel del propio tipo y exige que cada uno tenga entrada en `NOMBRE_DE_CAMPO`, con una lista corta de exclusiones (los de auditoría y los de máquina, que el historial no ofrece). Nació de este cambio y **no de un bug**: el diccionario es una lista escrita a mano al lado de un modelo que crece, y el que ve la clave cruda es el que está tratando de recuperar una descripción que pisó. MUTACIÓN PROBADA: sacando la entrada `comisiones`, el caso falla nombrándola |
| Que la restauración del historial arme el payload con el snapshot que la pantalla leyó al montar, y le devuelva al documento los `calendarEventId` de entonces | `tests/historial-relectura.test.ts` (**B-181**, con el argumento de B-150/D-360/D-91). Es la única cobertura de `restaurarCampo`, que hace I/O: dobla `leerActividad` y `updateDoc` y afirma **qué dato terminó en la escritura**, con el escenario de B-80 —la pantalla montada antes del write-back del sync— y su control negativo (si la relectura no trae nada, escribe con el snapshot). Nació de un chequeo **sobre la fuente** que el `auditor-trampas` rechazó por estar calcado de la implementación |
| Que restaurar del historial escriba un documento que rompa una regla del schema, o que «Restaurar → Estado» publique salteando el nivel de publicar | `tests/historial-restaurar.test.ts`, `describe('la restauración pasa por el schema (B-818)')` (**B-818**, D-540): trece casos sobre `issuesDeRestauracion` — el diff de rechazos antes/después, la excepción de los rechazos de privacidad cuando la restauración mueve el estado y su control negativo, el simétrico que despublica, la actividad ya rota que no tapia la pantalla, las dos ramas del documento ilegible, el par de B-181 sobre una publicada, el mensaje que corta en tres el barrido de centinelas sobre ese mensaje —con el valor restaurado **siendo** el centinela, porque la primera versión no podía fallar— y la otra mitad del §7.3, que es descancelar un encuentro. Y `tests/schema.test.ts` cierra el índice por arriba: deriva del comportamiento las reglas que solo corren con página y exige que estén declaradas como rechazos de privacidad, así que la regla que se agregue mañana entra al barrido sola. Y el cableado —que `restaurarCampo` la llame **antes** del `updateDoc`— va en `historial-relectura.test.ts`, doblando la escritura y afirmando que no ocurrió: un chequeo sobre la fuente es lo que el `auditor-trampas` ya rechazó en este mismo camino. **Y desde B-819 el archivo tiene además un control de *clase* sobre esa puerta:** `describe('B-819 — y la guarda se re-evalúa contra lo releído')` barre las llamadas del cuerpo de `restaurarCampo` y rechaza cualquiera que mire el snapshot del montaje en vez del documento releído. Nació porque la tercera guarda repitió el agujero de B-285 —lo señaló el docblock de `07-seguridad.md`, no un test— y el barrido lo cierra para la cuarta. El regex ignora comentarios y tolera un nivel de anidamiento en los argumentos: la versión simple cortaba en el primer `)`, así que una guarda con el primer argumento computado pasaba **en verde** mirando `actual`, que es el bug que el control existe para atajar (lo encontró el `auditor-trampas` atacando el chequeo mismo) |
| Que el agrupado por comisión de la página de detalle se decida en `detallePublico.ts` y la plantilla no lo pinte | El **paso 4b** de `scripts/build-contra-emulador.mjs` (**B-181**), sobre el **HTML de verdad**: que aparezca el encabezado de la opción y que el título de la sección pase a «Elegí tu opción». Es la mitad que ningún unitario puede mirar — el agrupado vive en `src/pages/actividad/[slug].astro` y un `.astro` no se importa desde vitest (D-140), así que `detallePublico.ts` afirma qué se decide mostrar y este gate es el único que ve si la plantilla lo pinta |
| Que un script que escribe se olvide la guarda de producción, o que no diga contra qué entorno está apuntando | `guardas-de-los-scripts.test.ts` (**B-630**, **B-810**). Dos clases en un archivo, las dos derivadas de `scripts/` y no de una lista: los que aceptan `--aplicar` tienen que exigir `--produccion` fuera del emulador, y los que **deciden** entre emulador y producción —los que usan `enEmulador`, siete hoy— tienen que anunciarlo con un `console.log` antes de escribir. Lo segundo nació de que `set-admin-claim.mjs`, el que reparte permisos, era el único que no lo hacía **mientras la doc afirmaba que sí**, y el dueño lo descubrió con un `USER_NOT_FOUND` crudo. El chequeo mira el código sin comentarios: dos mutaciones demostraron que buscar las palabras en el archivo pasaba con el anuncio borrado |
...
| Que el panel le muestre a una persona el mensaje crudo del SDK, en inglés | `fallos-del-panel.test.ts` (**B-929**), sobre `src/lib/fallosDelPanel.ts`. Lo que cierra no es el tipeo de una frase: es que **el panel ya sabía lo que había pasado y no lo usaba para hablar**. `clasificarFalloGuardado` mapeaba `unavailable` y `deadline-exceeded` a `motivo: 'red'` desde antes, así que la **métrica** quedaba bien etiquetada mientras la persona leía «Failed to get document because the client is offline». Por eso el módulo **no clasifica**: importa esa función y traduce su resultado, para que no haya dos clasificadores —uno para medir y otro para hablar— que se separen sin que nada falle (clase de B-88). Y la regla de qué se traduce está escrita como código y no como criterio: lo que `clasificarFalloGuardado` reconoce **por mensaje propio** devuelve ese mensaje —«Fecha inválida: "31/02"» nombra el dato y es accionable—, y lo que reconoce **por código de Firebase** devuelve texto nuestro. Es puro, así que se prueba sin DOM y sin Firestore |
...
| Que un **barrido que borra lo que leyó** se escape del chequeo de la clase de B-85, y que no se sepa qué protege a cada uno | `clases-de-bug.test.ts` (**B-867** y **B-862**). El síntoma de efecto era `/\.(set|update)\(/` y el de red conocía `fetch(`, `cal.events.` y `google.calendar(`: un barrido que **borra** era invisible —B-864 tenía la forma de B-85 con el verbo cambiado— y los clientes de GA4 y de Search Console daban `red: false`. **El alcance se midió antes de ensanchar y no después**, que es lo que los dos ítems pedían: con `.delete(` entran tres schedules (los tres barridos) más `borrarImagenAlCerrar`, con `google\.\w+\(` entra `traerAnaliticaDelSitio`, y **ninguno de los cinco queda en la clase** —a los barridos les falta la red, a la analítica la lectura—. Lo que sí cambió de sentido es la garantía: el caso que decía «los tres barridos pasan **porque borran**» congelaba la ceguera como promesa, y hoy dice que entran **con el borrado a la vista** y pasaban por no hablar con la red (hasta B-879), que es un motivo más débil y por eso no alcanza solo. La otra mitad es `GUARDAS_DE_BARRIDO`: cada barrido declara **precondición**, **generación** o **margen**, con la declaración **y el uso** anclados en el fuente, y la ventana intra-corrida marcada `cubierta` o `aceptada`. Un margen no puede declararse `cubierta` —es la confusión de partida: `MARGEN_DE_GRACIA_MS` protege de «esto recién se creó», no de «esto cambió mientras yo corría»— y aceptarla obliga a nombrarla y a escribir el motivo. El conjunto **se deriva del código**, así que un barrido nuevo se pone rojo hasta que alguien decida cuál es su guarda, y uno que deje de serlo también, para que no quede una fila contando una protección que ya nadie tiene. Doce mutaciones: revertir cada regex pone en rojo el caso que lo estrena —incluida la copia mala en su versión `delete`, que antes pasaba en verde—, un `fetch` en un barrido o un `.get()` en el schedule de analítica ponen en rojo el chequeo de B-85 **mismo**, y sacar la precondición de `retencion.js` o dejar de aplicar un margen pone en rojo la declaración. **Desde B-879 (D-1081)** no hablar con la red dejó de contar: el chequeo de B-85 exige la guarda declarada con sus marcas en el fuente, y un `fetch` solo lo pone en rojo en un barrido cuya guarda es un margen |
...
| Que un **flag de publicación** se escriba sin que la proyección lo mire antes del dato que esconde | `clases-de-bug.test.ts`, el `describe` de **B-911** (abierto por B-832, cerrado por B-833). La clase es «un booleano que decide si otro campo del mismo documento se publica», y hay cuatro instancias: `online.urlPublica` (D-15), `material.items[].publico` (§5.1), `envio.manda` (B-832) y `direccionPublica` (B-833, § 6 del PRD 4 — lo que esconde es la dirección de la casa de una persona). Hasta la tercera la cobertura era **por instancia** y dos docblocks llegaron a afirmar que había red: eso es exactamente el daño que el ítem describía. El registro es `src/lib/paresFlagDato.ts` y tiene **una mitad derivada y una a mano**, como `EFECTOS_INCONDICIONALES`: se deriva todo booleano de un tipo del modelo que se llame «público» —tiene que estar registrado o declarado en `NO_SON_PARES` con su motivo, así que un `xxxPublica: boolean` nuevo no entra en silencio— y se escribe a mano el que no se llama así (`envio.manda`, y extenderlo cuando aparezca otro es trabajo de los auditores). El chequeo exige que cada par tenga **una** función productora, que esa función **lea el flag** (`.manda`, no `manda` a secas: la primera versión buscaba el nombre pelado y una mutación la delató, porque `const manda = true` la dejaba en verde) y que el dato no se lea fuera de ella. Del mismo registro sale **qué campos vigila** `flagsDePublicacionRestaurables` (B-819), en vez de una lista de dos nombres escrita a mano. **Lo que el registro deja escrito y hoy no se puede hacer:** los pares de `/suscripciones` y `/lugares` no llegan a esa guarda porque esas colecciones no tienen subcolección `/versiones` (`conHistorial: false`, cruzado contra `firestore.rules`): no es que la guarda esté floja, es que la puerta no existe, y el día que un directorio gane historial el chequeo la pide en el mismo cambio |
...
| Que la sección comercial `/anunciar` insinúe un número de audiencia que no existe, niegue un tercero que el sitio sí tiene, prometa un plan o un precio, o escriba el `mailto:` a mano | `tests/comercial-del-sitio.test.ts` (B-770, D-450). Barre el texto de `comercialDelSitio.ts` —título y `meta description` incluidos— buscando la **forma** de una cifra de audiencia y una lista de palabras de volumen (la medición arrancó el 2026-09-03, B-372/B-373), con las mutaciones probadas y un caso por valor para la exención de los años; exige que la letra chica lo diga en vez de callarlo, prohíbe negar los terceros que el sitio tiene (`fonts.googleapis.com`, `gtag.js` — B-481, B-372), y verifica que no aparezca la clase B-72/B-88 (`mailto:` a mano) y que la página mande primero a `/contacto` a quien organiza gratis |
...
| Que el `dist/` que se publica **no lleve App Check**, y nadie se entere hasta que el panel deje de guardar | `scripts/verificar-bundle.sh`, la mitad simétrica que le faltaba al gate (**B-868**). Verifica sobre el artefacto que la clave de `.env.production` esté en un chunk, que **viaje** a `activarAppCheck` con `usarEmuladores` en falso —el aserto es sobre el objeto literal, y sigue la indirección si el bundler no inlinea la constante— y que el proveedor sea el de Enterprise. **El aserto del proveedor NO es `recaptcha/enterprise.js`**, que era el obvio y solo puede pasar: esa cadena y `recaptcha/api.js` las trae el **SDK de Auth** para sus propios flujos y están en el bundle con cualquiera de los dos proveedores (medido sobre dos builds el 2026-09-10). Lo que discrimina es el endpoint de canje: `exchangeRecaptchaEnterpriseToken` presente y `exchangeRecaptchaV3Token` ausente, y van los dos. **Va en el script y no en un test que lea `dist/`** —que era el precedente— porque en `deploy.yml` los tests corren antes del build y en `push-main.yml` en otro job que nunca buildea: un test así se saltearía en silencio justo donde tiene que morder |
| Que el build deje de emitir la página que **Firebase sirve como 404**, o emita una que no es ésa | `scripts/verificar-bundle.sh`, sección 4.1 (**B-310**, **B-880**). Firebase Hosting usa `404.html` de la raíz del directorio publicado: renombrar `404.astro` deja el build verde, el typecheck verde y todos los tests de contenido verdes —verificado de punta a punta— y lo único que pasa es que se vuelve al 404 de Firebase, con la página publicada en una URL que nadie visita. El gate exige además que sea **ésta**: el título, el `noindex`, el formulario por GET, el parámetro que la home lee (`q`) y el enlace a `/pasadas`, las tres cosas que el encabezado de `tests/no-encontrado.test.ts` enumera como rompibles en silencio. **Las cinco salen del fuente que las declara, no escritas en el script**, que es el patrón de la sección 2 con `.env.production`. **Esto vivía en un `it.skipIf(!hayBuild)` que era el último skip de la suite entera** (B-880) |
| Que las clases de la grilla del tríptico **no lleguen a la hoja construida**, y la home se quede en una columna | `scripts/verificar-bundle.sh`, sección 4.2 (**B-600**, **B-880**). Tailwind genera las utilidades leyendo el fuente: un mapa de literales que igual no ve —por vivir fuera de su `content`, o por una clase escrita mal— tiene el mismo aspecto en el fuente y el mismo bug en pantalla. Y la mitad que hace que signifique algo es el **marcador**: las cuatro utilidades del tríptico las escriben también otros archivos, así que el barrido pasaría verde con `components/sitio/estilos.ts` fuera del scan; lo que prueba que ese archivo se escanea es la clase de dos columnas de `CLASES_DE_PARED`, que solo él escribe. **Verificado de punta a punta sacando el archivo del scan: las cuatro utilidades siguieron en la hoja y lo único que se cayó fue el marcador.** Que la clase siga siendo exclusiva lo verifica `ahoraPublico.test.ts` con un grep al repo entero, que es sobre el fuente y no necesita build. **Esto vivía en un `if (hojas === '') return;`: sin `dist/` el caso no se salteaba, se reportaba PASSED habiendo leído cero bytes** (B-880) |
...
| Que **cualquier documento se renderice como código** por un ` ``` ` de sobra | `bloques-de-codigo-en-la-doc.test.ts` (**B-294**, la segunda cara del ítem). Aparecieron **seis** bloques así el 2026-09-07 —D-460 y D-461 completas, siete ítems del BACKLOG, las filas de B-770, trece ítems viejos y dos entradas del CHANGELOG: más de cuatrocientas líneas—. Misma causa que la fila de abajo: texto pegado desde un `.estado/*.md` con sus propias marcas de bloque. **Nadie lo veía**, y por tres motivos que conviene tener escritos: en un editor el texto se lee igual, el chequeo que había cuenta filas de tabla y no fences, y el `auditor-documentacion` lee el **contenido** —que está bien; lo que está mal es cómo se renderiza—. Van las dos mitades: «todo bloque cierra» agarra el fence sin pareja, y «ningún bloque contiene un encabezado ni una fila de tabla» agarra el daño real, que venía **en pares** y por eso se veía balanceado |
...
| Que un **fixture con fecha cableada** compita contra el reloj real, y un test se ponga rojo solo el día que esa fecha pasa | `fixtures-contra-el-reloj.test.ts`, el `describe` de la **clase de B-875**. El caso que lo disparó fue un encuentro del 10 de septiembre en `lista-actividades.render.test.tsx`: `ListaActividades` decide qué fila dice «Próximo:» mirando el reloj **adentro** del componente (`useState(() => new Date())`), así que el caso vivió en verde hasta esa fecha y se cayó solo — y con él ocho corridas del rebuild, que corre los tests **antes** del build y dejó de publicar lo que el dueño cargaba. **No prohíbe la fecha fija**, que en la lógica pura es justamente lo correcto (ahí el `ahora` entra por parámetro): busca la conjunción de tres señales —la fecha todavía no pasó, lo que el test monta lee el reloj real sin que nadie se lo pueda pasar, y el test no fijó el reloj del proceso—. Es de **clase**: el fixture que nazca así entra en rojo el día que se escribe, no el día que la fecha caduca. Y sus controles positivos prueban que el barrido sí encuentra las dos señales, para que la lista vacía sea evidencia y no un barrido que no mira nada (B-873) |
| Que el sitio público contacte a un tercero —`preconnect`, `script`, `iframe`— **antes** de que la persona decida sobre las cookies | `scripts/verificar-bundle.sh`, sección 3.2/3.3 (D-254). Exige lista blanca con motivo para todo host absoluto del HTML y de las hojas construidas. **Esta fila decía `terceros-antes-del-consentimiento.test.ts` y era falsa:** ese test leía el `dist/` y se salteaba sin build, o sea que no corría en CI ni una vez (**B-873**). Hoy el barrido vive en el gate —el único paso del pipeline donde `dist/` existe— y el test **maneja el script** sobre artefactos sintéticos, sin un solo `skipIf`. El hallazgo original fue un `preconnect` a `googletagmanager.com` sin condición: no es una pista pasiva —el navegador resuelve DNS, abre TCP y completa el handshake TLS con SNI en el load— así que le avisaba al borde de Google incluso de quien **rechaza**, contradiciendo la promesa de D-250. Lo encontró el coordinador con un `grep` a mano, que es exactamente lo que no da garantía la próxima vez |
...
| Que una fila de la tabla de salidas de la ficha del `auditor-privacidad`, de `07-seguridad.md` o del skill `campo-nuevo` tenga **menos columnas** que su encabezado, o que la prosa de cualquiera de los dos diga la cuenta vieja **en negrita** | `agentes-y-skills.test.ts` (**B-1942** y **B-1941**). La fila 30 (`/guia`) nació sin la columna «Test que la fija» y nada lo decía: el parseo busca la celda del productor por contenido y Markdown pinta la celda que falta vacía. Y el caso de la prosa buscaba `palabra salidas` exacto, así que la cuenta vieja escrita en negrita en el skill —con los `**` entre el número y la palabra— pasaba con la tabla en treinta y dos filas; ahora acepta la negrita y barre también el skill. Desde **B-1950** el conteo de columnas barre también `07-seguridad.md`, que tenía el encabezado en tres columnas y la de tests sin título; y corta en el `|` sin escapar, como GitHub, porque un `|` adentro de backticks también parte la celda |
...
| Que un campo privado **sobreviva en el artefacto que se sube**, en cualquier archivo del `dist/` | El **paso 9** de `scripts/build-contra-emulador.mjs` (**B-121**). Recorre todo `.html`, `.json`, `.xml` y `.txt` que el build escribió y barre los centinelas, con las excepciones declaradas por salida — **cuatro canastas desde B-804**, porque el monto lo imprime la **tarjeta compartida** y por lo tanto sale también en la home y en los hubs (D-500); esa cuarta va como lista y no como `else`, así que una página nueva que pinte la tarjeta entra en rojo hasta que alguien la declare. Es además el primer centinela **numérico** y el primero cuyo permiso depende de **cómo se escribe** (crudo al `events.json` y al `Offer`, legible a la página), con tres controles positivos: un barrido que solo afirma ausencias vuelve en silencio al estado del que viene. **No reemplaza a `barrido-de-salidas-publicas.test.ts` ni al revés:** aquél mira las funciones puras y corre sin build; éste mira lo que quedó escrito, que es lo único que prueba que ninguna plantilla interpoló por su cuenta. Va en el gate y no en la suite porque necesita un `dist/` (criterio de B-217) |
| Que aparezca un **ciclo de imports**, que `10-salud-del-codigo.md` nombre un archivo que ya no existe, o que su metodología escrita deje de ser la que aplica el script que mide | `salud-del-codigo.test.ts` (**B-311**). Las cifras de ese documento **no** se chequean, y está decidido: se mueven con cada commit de cualquier frente, así que un chequeo se pondría rojo en la rama de quien no tocó el documento — B-180. **La excepción son las dos alarmas del formulario** (B-856, B-1073): son de un archivo, y solo las mueve quien lo edita. Lo que se ata es lo discreto, y el ciclo es el único que además es una propiedad: no aparece por trabajo ajeno. Remedir es `node scripts/salud-del-codigo.mjs`, que avisa y no bloquea |
...
| Que el alta al correo cargue un **script de Mailchimp**, que su promesa se afloje, o que la sección se dibuje sin lista | `boletin-del-sitio.test.ts` (**B-847**, D-640). El embebido de Mailchimp trae `mc-validate.js` de `chimpstatic.com` en el load, y pegarlo es la forma más natural de romper D-254; `terceros-antes-del-consentimiento.test.ts` lo agarra **solo con un `dist/` construido**, así que éste lo afirma sobre el fuente y falla en el acto. Ata además las cinco promesas del trato una por una, que la cadencia **nombre la semana en que no sale** (un piso se incumple con una sola semana floja), que el texto **no** diga «tu dirección no sale de acá» —que es falso— y que `/ayuda` y `/apoyar` cuenten el correo **si y solo si** `hayBoletin()`. Lo que **no** puede sostener es el doble opt-in: es una casilla de la consola de Mailchimp, la misma clase que los ajustes de GA4 de B-480 |
| Que un test de reglas escriba su **propio** veredicto de rechazo en vez de pasar por el helper compartido | `rechazos-sin-copia.test.ts` (**B-1130**): barre `tests/` buscando el casteo `(e as { code?: string }).code` y la regex `RECHAZADA = /permission|insufficient/i`, con dos excepciones escritas a mano. **La clase que cierra no es el tipeo, y esta vez se puede medir:** las **nueve** copias que reemplazó afirmaban lo mismo con textos distintos y ninguna podía distinguir «la regla denegó» de «la regla **nunca contestó**» —un error de evaluación llega con el mismo `permission-denied`—, y la que se había propuesto como arreglo (la regex) matcheaba las dos cosas igual. El recuento a mano decía **seis**; las otras tres las encontró este barrido al correr, porque tres archivos tenían además del helper de escrituras uno aparte para lecturas. Probado por mutación en las dos direcciones: una copia sintética se marca, y **nombrar** el `code` en prosa —como hace el traductor de errores del panel— no alcanza para ser infractor. ⚠️ **Punto ciego declarado:** el `rejects.toThrow()` pelado, que es *más* débil todavía —lo satisface un emulador caído— y que por texto no se puede separar de los dos usos legítimos que quedan (Storage, que es otro emulador; y `opciones.integracion.test.ts`, donde lo que tira es el código del panel). Lo prueba el propio cierre de B-1130: cuatro instancias de esta clase quedaron sin migrar en archivos que el cambio tocó línea por línea, y las encontró el `auditor-trampas`, no la guarda. Anotado como **B-1132** |
| Que una imagen commiteada a `public/` salga a una página indexada **con sus metadatos**, o declarando medidas que no son las suyas | `banner-de-ciudad.test.ts` + `webp.test.ts` (**B-962**): el chequeo abre el archivo, enumera sus chunks RIFF y compara las medidas reales contra las declaradas. **Es el único camino por el que una imagen de un tercero llega a HTML indexado sin pasar por ningún saneador** — la foto de una propuesta pierde el EXIF en `flyer-de-propuesta.js` (B-896) y lo del bucket se recomprime, pero un archivo commiteado no pasa por nada: el barrido del build mira `.html`/`.json`/`.xml`/`.txt`, no binarios. Hasta acá la garantía era que quien lo commitea se acordó de convertirlo. Y la mitad de las medidas tapaba un caso real: el chequeo viejo comparaba la **relación**, y 2400×600 declarados sobre un archivo de 1600×400 dan 4:1 = 4:1 —pasa en verde mientras el navegador reserva un alto que la imagen no tiene (§CLS)—. Probado por mutación en los dos ejes: un WebP sintético con `EXIF`, `XMP ` o `ICCP` se marca, uno con la relación correcta y las medidas distintas también, y un formato que el lector **no sabe leer** (`VP8L`) se reporta en vez de darse por bueno. Los dos hallazgos son del `auditor-privacidad` sobre el cambio que estrenó la primera fila |
...
- **Un auditor del sitio público** — ✅ **decidido que NO, el 2026-09-03 (B-122).**
  ...
  | El **nombre accesible** de un control, y el **foco en un recorrido real** | el foco, **nada**; el nombre accesible de un **campo de formulario**, sí desde **B-827** (`clases-de-bug.test.ts` exige el par `htmlFor`/`id`) | el foco sigue siendo manual — ver abajo |
  ...
  **Lo que queda no lo hace un agente tampoco, y por eso el ítem cierra en vez de
  ...
  allá del contraste (orden de encabezados, foco en un
  recorrido real) —**los nombres accesibles de los campos de formulario salieron
  de esta lista con B-827**: el `htmlFor` de `Campo` es requerido y
  `clases-de-bug.test.ts` exige que el hijo lleve ese id, así que esa mitad ya
  tiene red; lo que queda sin ella es el nombre accesible de lo que no es un
  campo— y la **calidad** del contenido indexable —que un `<title>` diga
