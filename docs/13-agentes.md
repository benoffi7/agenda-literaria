...
|---|---|---|---|
| 🔒 | `auditor-privacidad` | agente (solo lectura) | Que nada privado llegue a las veintiuna salidas públicas |
...
**Para qué.** El proyecto tiene **veintiuna salidas públicas** y una sola regla
...
tocados contra las veintiuna salidas, un bloque por hallazgo (severidad P0/P1/P2,
...
arranca obligando a decidir las veintiuna salidas **antes** de escribir código, que
...
| Que se publique una librería y el sitio estático **no la muestre nunca**, o que cada publicación cueste dos builds | `directorios-rebuild.test.ts` (**B-901**, trampa 8). `functions/` no puede importar de `src/` (D-20), así que `CAMPOS_PUBLICOS` es una **segunda escritura** de lo que la proyección publica: el test las compara, y la mutación —sacar `'barrio'`— deja en rojo el caso nombrando el campo. El efecto real de esa divergencia es mudo en las dos direcciones: un campo de menos y corregir el barrio de una librería publicada no llega nunca al sitio; un campo de más y el write-back de un trigger dispara un build por sí solo. Cubre también la guarda contra el write-back de `publicadaAlgunaVez` —el trigger que falta (B-905)— y que corregir el **contacto interno**, que no sale al sitio, no cueste un build |
