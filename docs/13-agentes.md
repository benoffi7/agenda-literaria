...
| Que ningún **markup del sitio** —`src/pages`, `src/components/sitio`— atenúe texto por debajo del piso de WCAG AA **sobre el papel** | `contraste-del-sitio.test.ts` (B-235). Los ratios salen de los tokens de `global.css` —parseados, no copiados, así cambiar la paleta se nota— y falla si algún archivo escribe un `text-tinta/NN` por debajo del piso, con control positivo para que una lista vacía no signifique «no leí nada». La matemática está aparte, en `contraste.test.ts` (B-243). Es la clase, no la instancia: la paleta puede estar perfecta y el componente siguiente bajarla igual, que es justo lo que había pasado |
| Lo mismo para la **grilla del listado**, que ninguna de las tres cosas que aquél mide le sirven | `tarjeta-del-listado.test.ts` (B-247, D-142/D-143). Aquél barre `src/pages` y `src/components/sitio`, mide solo `text-tinta/NN` y solo sobre `papel`; la grilla vive en `src/components/publico/*.tsx`, se apoya en `crema` y `hondo` —donde el mismo `/65` pasa de 5,26:1 a 5,04:1— y su portada pinta texto encima del **color del tipo**, que no es un token sino un `oklch` derivado del slug. Tiene una regla de clase (la única atenuación permitida es la de la tinta, medida contra la superficie más oscura) y una lista de los pares de color que el arte declara, cada uno con la clase que tiene que **seguir estando** en su archivo para que la lista no envejezca dando cobertura de mentira. Encontró el chip elegido en 4,38:1 el día que se escribió. Fija además lo que se puede leer del markup: que ningún `div` lleve `onClick`, que nadie apague el anillo de foco, y qué campos de la entrada puede tocar la tarjeta |
| Que ningún componente del sitio público use texto por debajo del piso de contraste de WCAG AA, **sobre cualquiera de sus superficies** | `contraste-del-sitio.test.ts` (B-235/B-243): calcula los ratios desde los tokens de `global.css` —parseados, no copiados, así cambiar la paleta se nota— y falla si algún archivo del sitio escribe una clase por debajo del piso, con control negativo para que el piso signifique algo. **Medía solo contra `papel`**, y con las tres superficies de D-141 eso da un número optimista: `text-tinta/61` pasa sobre papel (4,62) y no sobre la más oscura (4,38). `contraste-de-superficies.test.ts` (**B-256**) mide lo mismo contra las tres y contra los tintes de acento, **derivados del markup y de la hoja de estilos** en vez de enumerados, así que una superficie nueva y más oscura entra sola al cálculo. Cubre además el acento como texto y los 360 tonos de tipo posibles. Es la clase, no la instancia: la paleta puede estar perfecta y el componente siguiente bajarla igual, que es justo lo que había pasado |
...
- **Un auditor del sitio público** (SEO, indexabilidad, accesibilidad): desde
  ...
  calculan `tests/contraste-del-sitio.test.ts` (B-235) para el markup del sitio y
  `tests/tarjeta-del-listado.test.ts` (B-247) para la grilla del listado, que es
  donde hay texto encima de algo que no es el papel. B-122.
  calculan `tests/contraste-del-sitio.test.ts` y, sobre las tres superficies,
  `tests/contraste-de-superficies.test.ts` (B-243, B-256). B-122.- **Un agente de code review genérico**: es exactamente lo que Claude hace sin
