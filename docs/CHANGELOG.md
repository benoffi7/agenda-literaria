...
**B-168** (el detector apagado), **B-117**, **B-50**, **B-119** y **B-115**, y
cubre la **trampa 4** del §13.

**B-168 · el detector de triggers blindados estaba apagado.** El chequeo de la
...
`it.skip` volvió a `it` y el `it.fails` de B-82 pasó a `it` (D-102).

Y va lo que faltaba la primera vez: **nueve tests del propio detector** contra
cuerpos sintéticos, incluida la regresión exacta de B-168, más un control
...
un `npm run build` de una vez (D-100).
...
algo ya cubierto (D-101). Quedó abierta la **trampa 7** (query pública sin el
`where`), anotada como B-167.
...
Anotado y **no** hecho, porque toca archivos de otros frentes: **B-167** (la
trampa 7), **B-169** (`tsc --noEmit` sale siempre en rojo por doce errores de
