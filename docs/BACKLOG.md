...
| Qué | Causa | Dónde |
|---|---|---|
| El bloque «Llevate la agenda a tu calendario» caía **pegado** a la fila «Mes por mes» de la tira «Explorá por» en la home | `SuscribirseResumen` no trae margen propio —a propósito, porque se reusa en `/suscribirse` donde abre la columna sin separación— y en la home iba puesto justo debajo de `ExploraPor` sin nada en el medio, mientras `ExploraPor` sí trae `mt-12 sm:mt-16` sobre el listado. Se veía en producción tras el deploy de los hubs. Cerrado envolviendo el uso en la home con `mt-12 sm:mt-16` —el mismo ritmo—, en el sitio de uso y no en el componente compartido | B-482, `src/pages/index.astro` (2026-09-03) |
