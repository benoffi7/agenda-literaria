...
| El bloque «Llevate la agenda a tu calendario» caía **pegado** a la fila «Mes por mes» de la tira «Explorá por» en la home | `SuscribirseResumen` no trae margen propio —a propósito, porque se reusa en `/suscribirse` donde abre la columna sin separación— y en la home iba puesto justo debajo de `ExploraPor` sin nada en el medio, mientras `ExploraPor` sí trae `mt-12 sm:mt-16` sobre el listado. Se veía en producción tras el deploy de los hubs. Cerrado dándole a `SuscribirseResumen` una prop `class` y pasándole `mt-12 sm:mt-16` desde la home. El primer intento fue un `<div>` envolvente y lo frenó el CI: `suscribirse.test.ts` cuenta etiquetas de apertura/cierre para verificar que el bloque quede fuera del contenedor de la island, y el wrapper desbalanceaba la cuenta —además, ese test cuenta también dentro de los comentarios, así que el comentario que lo explicaba tuvo que dejar de nombrar la etiqueta literal— | B-482, `src/pages/index.astro` (2026-09-03) |
