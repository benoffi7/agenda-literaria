...
> ⚠️ **Leer junto con B-1021, escrito el mismo día y unas horas después.** La
> medición de abajo se hizo **desde un worktree**, y desde ahí se descubrió
> después que `rol-publicador.integracion.test.ts` —el otro archivo que pasa los
> claims **por el registro**— también falla, mientras que en el árbol principal y
> en CI pasa. O sea que puede que las dos sean la misma cosa y que ninguna sea de
> producción: un artefacto de correr contra un emulador levantado en otro
> directorio, con otro `projectId`. **Lo que sí sigue en pie, y es lo que hace
> que este ítem no se descarte:** `storage-reglas.integracion.test.ts` pasa
> 19/19 **sin modificar** desde ese mismo worktree, y solo se rompe cuando se le
> saca el claim del token. El síntoma es real; lo que está en duda es si el
> sujeto es Storage o es el emulador. **Primer paso, antes que cualquier otro:**
> rehacer la mutación **en el árbol principal**. Si ahí pasa, esto se cierra
> junto con B-1021 y nada de lo de abajo aplica a producción.
