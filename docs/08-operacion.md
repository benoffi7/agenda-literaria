...
| Un push a `main` no publicó nada y la corrida figura `startup_failure` en 0s sin jobs | falla del lado de GitHub: el workflow no llega a crear los jobs. Verificar que no sea del repo (el YAML parsea, el workflow está `active`, Actions habilitado) antes de buscar en otro lado | **no se puede reintentar** esa corrida, y el push siguiente **no lo repara** porque `decidir` diffea desde el commit ya subido (**B-205**). Republicar con `gh workflow run deploy.yml --ref main`, que es Hosting solo; y confirmar con `curl -s https://agenda-literaria.web.app/version.json`, no con el color de la corrida |
