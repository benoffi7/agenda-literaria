...
| `contacto` es **el primer dato personal de un tercero** que el proyecto guarda | B-102 decía que no guardaba ninguno. Se reabre a propósito: sin forma de repreguntar la bandeja no sirve. No sale a ninguna salida, tiene su fila en [`07-seguridad.md`](07-seguridad.md) y su retención a 30 días, **escrita y desplegándose con este commit**: `borrarPropuestasVencidas` borra la rechazada y su imagen a los 30 días del rechazo (DEC-13, B-838; la ventana entre «escrita» y «corriendo» está en [`07-seguridad.md`](07-seguridad.md)). **Y desde B-844** (2026-09-09) la `nueva`/`en-revision` sin tocar caduca a los **30 días de su última señal de vida** —el máximo entre `creadoEn` y `revision.en`, así que moverla de estado le renueva el plazo, y desde **B-864** eso vale también si el movimiento cae **mientras el barrido corre**: borra con precondición—, también con su imagen. La `aceptada` **no vence**, y eso es una decisión escrita (ahí el contacto sirve). Lo que conserva sin plazo es **el contacto y nada más**: la foto original se borra al aceptar, cuando la copia promovida ya la reemplaza (**B-863**, decisión del dueño del 2026-09-10) **o cuando quien revisa eligió no usarla** al convertir (**B-926**), que es el único caso en que se borra sin que exista ninguna copia — ahí la pregunta que la verificación hace ya la contestó una persona mirando la foto. Hasta ese ítem la decisión se había tomado mirando un campo y se aplicaba a dos. **Y si al aceptar la actividad todavía no tenía copia** (se guardó sin foto y el flyer se subió a mano después), el original ya no queda para siempre: `borrarPropuestasVencidas` lo borra en su corrida diaria una vez que la actividad tiene su imagen propia y viva, con la misma verificación de B-863 (**B-1370**, D-890) |
| `imagen` es **una URL de afuera o un objeto nuestro, nunca las dos** | DEC-11. El objeto vive en `propuestas/`, que no es público (`get` solo para un admin, `list` para nadie): su ciclo lo decide el estado — se promueve a `imagenes/` al convertir y el original se borra **en los tres cierres**: al rechazar, en el acto; al aceptar, después de verificar que la copia exista (**B-863**); y al descartar la foto al convertir (**B-926**), sin verificar nada porque no hay ninguna copia. Si el de la aceptada se conservó igual —sin copia, o porque el borrado falló—, se va **a los 30 días de aceptada** (**B-871**), aunque el documento no venza. Ver `08-operacion.md` § «La imagen de una propuesta» |
...
| `direccion`, `provincia`, `barrio`, `ciudad`, `geo` | los tres de la geografía son la **misma cascada que una sede** (B-967, D-710 § 8): la provincia se exige, y se pide el barrio en CABA o la ciudad en el resto. Los tres son el mismo vocabulario que usan las actividades, que es lo que permite cruzarlas en los hubs |
...
| `direccion`, `provincia`, `barrio`, `ciudad`, `geo` | ⚠️ la `direccion` y la `geo` **pueden no publicarse**. Los tres de la geografía son la misma cascada que una sede y una librería desde B-967 (D-710 § 8), con el mismo vocabulario que las actividades —lo que deja que los hubs crucen las tres cosas— y salen **siempre**, también para una casa: son el «más o menos por Villa Crespo» que el § 6 sí autoriza |
...
| `horarios` | ⚠️ **texto libre y opcional** (B-982), y acá el rótulo es otro: en un lugar para eventos se pregunta **cuándo se puede usar**, que no es un horario de mostrador. **No depende de `direccionPublica`**, y ésa es la decisión del campo: la calle y el número identifican una casa, *cuándo se puede usar* no — así que una casa con la dirección reservada igual puede decir cuándo está disponible, que es justamente lo que esa persona sí quiere decir. Tampoco se emite como `openingHours`, por el mismo motivo que en una librería |
...
| `inscripcion.destino` con `via: 'dm'` (actividad) | **Ninguno al cargar, deriva al mostrar**: se guarda recortado (`limpiar`) y el schema solo exige que no esté vacío al publicar. Las tres salidas que lo muestran lo resuelven con el saneador: `accionDeInscripcion` (`src/lib/detallePublico.ts`) para el botón, `destinoLegible` (`src/lib/textoRedes.ts`) para el posteo y `arrobaPublicable` (`functions/calendario.js`) para el evento, **solo con `via === 'dm'`** | el de Instagram, vía `handleInstagram` / `arrobaInstagram` | se guarda crudo (recortado). La ficha no arma botón y muestra el canal como texto; el posteo y el evento lo publican **como se escribió, sin arroba**. Lo que sí se reconoce sale `@handle`, sin el `?igsh=…`. Al `events.json` va tal como está guardado | B-1540: es el criterio de D-763 (derivar al mostrar, sin reescribir el documento) extendido a un campo que no dice «instagram» en el nombre, y solo con esa vía, porque con WhatsApp o formulario un `1155556666` o un `casa.brandon` pasarían el alfabeto sin ser cuentas. D-985 |
