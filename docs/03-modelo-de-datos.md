...
| `imagen` es **una URL de afuera o un objeto nuestro, nunca las dos** | DEC-11. El objeto vive en `propuestas/`, que no es público (`get` solo para un admin, `list` para nadie): su ciclo lo decide el estado — se promueve a `imagenes/` al convertir y el original se borra **en los dos cierres**: al rechazar, en el acto; al aceptar, después de verificar que la copia exista (**B-863**). Ver `08-operacion.md` § «La imagen de una propuesta» |
