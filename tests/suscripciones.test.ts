...
/**
 * **La puerta anónima de `/suscripciones`, del lado que no necesita emuladores** —
 * D-700, 2026-09-15.
 *
 * El control positivo de la puerta y sus negaciones viven en
 * `tests/suscripciones.integracion.test.ts`, que es donde se pueden afirmar de verdad.
 ...
 * justamente por eso.
 */
describe('el `create` público de /suscripciones — D-700', () => {
  it('está abierto: el `create` ya no exige `esAdmin()`', () => {
    expect(REGLAS).toContain('allow create: if suscripcionValida();');
    expect(REGLAS, 'la puerta volvió a cerrarse y la doc sigue diciendo que está abierta')
      .not.toContain('allow create: if esAdmin() && suscripcionValida();');
  });
  ...
  it('y la ficha que llega de afuera nace sin fotos', () => {
     ...
     * `tests/suscripciones.integracion.test.ts`.
     *
