...
describe('la portada pinta la miniatura como candidato de srcset — B-321', () => {
  it('el src queda en el original, la miniatura solo como candidato chico', () => {
     ...
     * condicionado a que exista de verdad: `urlMiniaturaPortada` llega `null`
     * para las externas (DEC-7d) y para las que el build no confirmó contra
     * Storage (D-210).
     *
  ...
  it('importa la composición del srcset en vez de reimplementarla — clase de B-88', () => {
    ...
    expect(src()).toContain("import { srcsetDeMiniatura } from '@/lib/imagenes'");
  });

  it('no deriva la miniatura acá: la recibe resuelta en las props — D-210', () => {
    /*
     * El defecto real de la primera versión era `urlDeMiniatura(portada.url)`
     * en esta misma plantilla: deriva la URL sin confirmar que el objeto
     * exista, y un `srcset` cuyo candidato da 404 **rompe** la imagen —no
     * degrada al original— porque una vez que el navegador elige un candidato,
     * esa URL reemplaza al `src`.
     *
     * La segunda versión confirmaba **acá**, contra un `Set` que venía en las
     * props. Lo corrigió el `auditor-privacidad`: ese set es la enumeración de
     * `miniaturas/` entero (borradores incluidos) y no tenía por qué entrar al
     * scope de una página indexada. Hoy la confirmación vive en
     * `caminosDeDetalle` y acá llega un `string | null`, así que esta plantilla
     * **no deriva nada** — que es lo que D-140 pide de ella.
     *
     * MUTACIÓN PROBADA: volver a derivar acá (con cualquiera de las dos
     * funciones) deja este test en rojo, y el del import también.
     */
    const cuerpo = sinComentarios(src());
    expect(cuerpo, 'la portada llega resuelta, no se deriva en la plantilla').not.toMatch(
      /urlDeMiniatura(SiExiste)?\s*\(/,
    );
    expect(cuerpo).toContain('const { detalle, urlMiniaturaPortada } = Astro.props as Props;');
  });
});
