...
/**
 * **La puerta anónima de `/librerias`, del lado que no necesita emuladores** —
 * D-700, 2026-09-15.
 *
 * El control positivo de la puerta y sus negaciones viven en
 * `tests/librerias.integracion.test.ts`, que es donde se pueden afirmar de verdad.
 ...
 * justamente por eso.
 */
describe('el `create` público de /librerias — D-700', () => {
  it('está abierto: el `create` ya no exige `esAdmin()`', () => {
    ...
    expect(REGLAS, 'la puerta volvió a cerrarse y la doc sigue diciendo que está abierta')
      .not.toContain('allow create: if esAdmin() && libreriaValida();');
  });
