 ...
 *
 * ── Por qué vive en `functions/` (B-893) ──────────────────────────────────
 * La callable que crea la etiqueta de un publicador (`alta-de-opcion-trigger.js`)
 * marca la opción con esta huella, y el panel del publicador la reconoce como
 * suya con **la misma** (`opcionesVisibles`). Si las dos mitades derivaran por su
 * cuenta y se separaran, la etiqueta recién creada **desaparecería del
 * desplegable de quien la creó** —ni aprobada ni suya— sin ningún error. Por eso
 * una implementación y `src/lib/huella.ts` como fachada, igual que `slugify`
 * (D-20).
 *
 * @param {string} uid
 * @returns {string}
 */
export const huellaCreador = (uid) => {
